...
def user_info_page():
    show_logo()
    title = "Before we begin, I’d like to know a little about you."
    st.subheader(title)
    voice_say_current(translate_ui_text(title), "voice_user_info_intro")
    if voice_enabled():
        st.caption("Voice mode is active. For this page, age and gender remain tap/type inputs for safety and accuracy.")
...
def symptom_category_page():
    ...
    if voice_enabled():
        voice_say_current("Please tell me what is bothering you today, or choose the closest category below.", "voice_symptom_category_intro")
...
def symptom_free_input_page():
    ...
    voice_text = ""
    if voice_enabled():
        voice_text = voice_record_to_state("voice_free_symptoms", "Speak your main symptom, for example: my body is hot")

    with st.form("free_input_form"):
        text = st.text_input("Your symptoms", value=voice_text or "")
