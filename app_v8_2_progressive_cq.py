...
def render_cookie_banner() -> None:
    ...
    <div class="referral-block">
      ...
      We use necessary cookies to keep the app working well, enable necessary cookies to proceed.
      Optional cookies can remember returning users so they don't have to re-enter their details.
    </div>
    """, unsafe_allow_html=True)

    c1, c2 = st.columns(2)
    with c1:
        if st.button("Accept optional cookies", key="cookie_opt", use_container_width=True):
            set_cookie_pref("optional")
            get_or_create_device_id()
            st.rerun()

    with c2:
        if st.button("Use necessary cookies only", key="cookie_nec", use_container_width=True):
