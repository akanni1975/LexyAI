...
try:
    from voice_support import (
        OPENING_TEXT, SAFETY_TEXT, mic_capture, parse_yes_no,
        render_voice_controls, selected_language, speak_text, translate_ui_text,
        voice_enabled, match_option, t,
    )
except Exception:  # Voice layer must never break the core clinical app.
    OPENING_TEXT = {"en": ""}
    SAFETY_TEXT = {"en": ""}
    def voice_enabled(): return False
    def selected_language(): return "en"
    def render_voice_controls(location: str = "main"): return None
    def speak_text(text: str, key: str = "voice_speak", lang: str | None = None): return None
    def mic_capture(key: str, prompt: str = "Record voice"): return None
    def parse_yes_no(transcript: str, lang: str | None = None): return None
    def translate_ui_text(text: str, lang: str | None = None): return text
    def match_option(transcript: str, options): return None
    def t(key: str, lang: str | None = None): return key
...
def voice_say_current(text: str, key: str) -> None:
    """Speak text only when Voice Lexy Beta is enabled."""
    if voice_enabled() and text:
        speak_text(text, key=key, lang=selected_language())


def voice_record_to_state(key: str, prompt: str = "Record your answer") -> str:
    """Capture a voice answer and persist the transcript in session state."""
    transcript = mic_capture(key=key, prompt=prompt) if voice_enabled() else None
    if transcript:
...
def render_admin_sidebar():
    with st.sidebar:
        render_voice_controls("sidebar")
        st.markdown(f"### {t('admin')}")
        pwd = st.text_input(t("admin_code"), type="password", key="admin_pwd")
        if pwd == DEV_PASSWORD:
...
def render_cookie_banner() -> None:
    ...
    <div class="referral-block">
      <strong>{t("cookie_notice_title")}</strong><br>
      <p style="margin: 8px 0 0 0;">{t("cookie_notice_1")}</p>
      <p style="margin: 4px 0 0 0;">{t("cookie_notice_2")}</p>
    </div>
    """, unsafe_allow_html=True)

    c1, c2 = st.columns(2)
    with c1:
        if st.button(t("remember_me"), key="cookie_opt", use_container_width=True):
            set_cookie_pref("optional")
            get_or_create_device_id()
            st.rerun()

    with c2:
        if st.button(t("accept_cookies"), key="cookie_nec", use_container_width=True):
...
def welcome_page():
    ...
    render_voice_controls("welcome")
    if voice_enabled():
        lang = selected_language()
        opening = OPENING_TEXT.get(lang, OPENING_TEXT.get("en", ""))
        safety = SAFETY_TEXT.get(lang, SAFETY_TEXT.get("en", ""))
        st.info(opening)
        voice_say_current(opening + " " + safety, "voice_welcome_opening")
    ...
    **{t("emergency_heading")}**
    ...
    if pref not in {"necessary", "optional"}:
        st.warning(t("cookie_required"))
        return

    confirm = st.checkbox(t("consent_checkbox"))
    if st.button(t("start_symptom_check"), disabled=not confirm):
...
def user_info_page():
    show_logo()
    title = t("user_info_title")
    st.subheader(title)
    voice_say_current(translate_ui_text(title), "voice_user_info_intro")
    if voice_enabled():
        st.caption(t("voice_mode_active_info"))
    with st.form("user_info_form"):
        ...
        gender_display = st.radio(t("gender"), [t("male"), t("female")], horizontal=True)
        gender = "Male" if gender_display == t("male") else "Female"
        conditions = st.text_input(t("existing_conditions"), placeholder=t("existing_conditions_placeholder"))
        submit = st.form_submit_button(t("continue"))
    if st.button(t("back")):
        st.session_state.page = "welcome"
        st.rerun()
...
def symptom_category_page():
    show_logo()
    st.subheader(t("symptom_category_title"))
    if voice_enabled():
        voice_say_current(t("symptom_category_voice"), "voice_symptom_category_intro")
    ...
    <div class="free-text-cta">
      <strong>{t("type_symptoms_cta")}</strong>
    </div>
    """, unsafe_allow_html=True)

    if st.button(t("type_my_symptoms"), key="free_text_top", use_container_width=True):
        st.session_state.page = "symptom_free_input"
        st.rerun()

    st.markdown(f"<div class='category-section-title'>{t('or_choose_category')}</div>", unsafe_allow_html=True)
...
def symptom_free_input_page():
    show_logo()
    title = t("what_are_symptoms")
    st.subheader(title)
    st.caption(t("symptom_caption"))
    voice_say_current(translate_ui_text(title), "voice_free_input_prompt")

    voice_text = ""
    if voice_enabled():
        voice_text = voice_record_to_state("voice_free_symptoms", t("speak_main_symptom"))

    with st.form("free_input_form"):
        text = st.text_input(t("your_symptoms"), value=voice_text or "")
        submit = st.form_submit_button(t("search_symptoms"))
...
def clarifying_questions_page():
    ...
    voice_say_current(qtext, f"voice_cq_speak_{qid}_{step_index}")
    voice_answer = ""
    if voice_enabled():
        voice_answer = voice_record_to_state(f"voice_cq_{qid}_{step_index}", "Speak your answer")
        if voice_answer:
            if rtype == "yes_no":
                parsed = parse_yes_no(voice_answer)
                if parsed:
                    saved_value = parsed
            elif rtype == "single_select":
                parsed = match_option(voice_answer, item["options"] or ["Not sure"])
                if parsed:
                    saved_value = parsed
            elif rtype == "free_text_short":
                saved_value = voice_answer
...
def risk_flag_selection_page():
    show_logo()
    risk_title = "These factors can affect your care. Select any that apply, or None."
    st.subheader(risk_title)
    voice_say_current(risk_title, "voice_risk_intro")
...
def anchor_gate_page():
    ...
    voice_say_current(question, "voice_anchor_question")
    voice_anchor_answer = voice_record_to_state("voice_anchor_answer", "Speak Yes, No, or Not sure") if voice_enabled() else ""
    default_anchor = parse_yes_no(voice_anchor_answer) if voice_anchor_answer else None
    anchor_options = ["Yes", "No", "Not sure"]
    anchor_index = anchor_options.index(default_anchor) if default_anchor in anchor_options else 0
    answer = st.radio(question, anchor_options, index=anchor_index, horizontal=True)
...
def results_page():
    ...
    result_title = "Based on your answers, this may be:"
    ...
    if voice_enabled():
        ...
        voice_say_current(spoken_result, "voice_results_summary")
