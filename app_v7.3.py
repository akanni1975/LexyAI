...
BODY_PART_SYNONYMS = {
    ...
    "breast":       {"aliases": {"breast","breasts","nipple","nipples","areola"}, "category_hints": ["Women’s Health","Swelling"]},
    ...
    "groin":        {"aliases": {"groin","crotch"}, "category_hints": ["Urinary & Sexual Health","Men’s Health","Women’s Health"]},
    ...
    "male_genitals":   {"aliases": {"penis","testicle","testicles","scrotum","balls","manhood","dick"}, "category_hints": ["Men’s Health","Urinary & Sexual Health"]},
    "female_genitals": {"aliases": {"vagina","vaginal","vulva","labia","clitoris","womanhood","private part"}, "category_hints": ["Women’s Health","Urinary & Sexual Health"]},
...
def symptom_free_input_page():
    ...
    def _ck_score_categories_and_subset(free_text, df, pri_col="Primary Category", sym_col="Symptoms"):
        ...
        # must have ≥1 symptom hit; block category-only admits
        current_gender = st.session_state.user_data.get('gender')
        current_age = st.session_state.user_data.get('age')
        ...
        if _flag("STRICT_GENERIC_MODE") and generic_only:
            ...
            # Dynamic, generic-only gating (no flags) — keep it narrow for the present generic(s)
...
def symptom_subcategory_page():
    st.image(logo, width=80)
    st.subheader("Let’s get a bit more specific")
...
def show_no_match_ui():
    st.warning(
        "I couldn’t confidently match your answers to a condition in this section. "
...
def risk_flag_selection_page():
    ...
    if len(flagged) == 1:
        ...
        # No CQ2 flags → pick the top-ranked candidate overall
        subset_ranked = (
            subset
            .assign(__rk=subset.apply(_rank_key, axis=1))
            .sort_values("__rk", ascending=True)
