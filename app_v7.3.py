...
LEXCONSULT_URL = "https://lexconsult.carekonnect.net/"


def render_help_link(label: str = "📅 Schedule a Consultation or Get Help", *, key: str | None = None):
    """
    Render a real outbound help/scheduling link.
    Uses st.link_button when available, otherwise falls back to a markdown link.
    """
    try:
        st.link_button(label, LEXCONSULT_URL, use_container_width=True)
    except Exception:
        st.markdown(
            f'<a href="{LEXCONSULT_URL}" target="_blank" '
            f'style="display:inline-block;width:100%;text-align:center;'
            f'padding:0.75rem 1rem;border-radius:8px;'
            f'background:#e63946;color:#fff;text-decoration:none;font-weight:600;">{html.escape(label)}</a>',
...
def _stem_light(word: str) -> str:
    w = _normalize_symptom_text(word)
    if not w:
        return w
    for suf in ("ing", "ness", "ment", "tion", "ions", "ion", "ed", "es", "s"):
...
def _symptom_variants(term: str) -> set[str]:
    t = _normalize_symptom_text(term)
    if not t:
        return set()
    ...
    for p in parts:
        out.add(p)
        out.add(_stem_light(p))
        for fam in GENERIC_FAMILY.values():
            if p in fam or _stem_light(p) in fam:
                out |= fam
    out.add(_stem_light(t))
    return {x for x in out if x}


def _symptom_similarity(a: str, b: str) -> float:
    a_n = _normalize_symptom_text(a)
    b_n = _normalize_symptom_text(b)
    if not a_n or not b_n:
        return 0.0
    if a_n == b_n or a_n in b_n or b_n in a_n:
        return 1.0

    a_parts = [p for p in re.split(r"[\s\-/]+", a_n) if p]
    b_parts = [p for p in re.split(r"[\s\-/]+", b_n) if p]
    a_set = set(a_parts) | {_stem_light(p) for p in a_parts}
    b_set = set(b_parts) | {_stem_light(p) for p in b_parts}
    ...
    seq_score = SequenceMatcher(None, a_n, b_n).ratio()
    return max(token_score, seq_score)


def _selected_matches_atom(atom_text: str, selected_symptoms: set[str], threshold: float = 0.72) -> bool:
    atom = _normalize_symptom_text(atom_text)
    if not atom:
        return False
    atom_variants = _symptom_variants(atom)
    for sym in selected_symptoms:
        sym_n = _normalize_symptom_text(sym)
        if not sym_n:
            continue
        if sym_n == atom or sym_n in atom_variants or atom in _symptom_variants(sym_n):
            return True
        if _symptom_similarity(sym_n, atom) >= threshold:
            return True
    return False


def _row_selected_overlap_score(selected_symptoms: set[str], row_symptom_text: str) -> tuple[float, list[str]]:
    row_syms = [s.strip() for s in str(row_symptom_text or "").split(",") if s and s.strip()]
    total = 0.0
    matched = []
    for row_sym in row_syms:
        best = 0.0
        for sel in selected_symptoms:
            best = max(best, _symptom_similarity(sel, row_sym))
        if best >= 0.72:
            total += best
            matched.append(row_sym)
...
def clarifying_questions_page():
    ...
    if selected_syms:
        def _row_matches(row):
            score, _matched = _row_selected_overlap_score(selected_syms, row.get("Symptoms", ""))
            return score > 0
...
def show_no_match_ui():
    ...
    with col1:
        render_help_link("📅 Get help / Schedule a Consultation")
    with col2:
...
def risk_flag_selection_page():
    ...
    def _rank_key(r):
        ...
        rule_len = len(str(r.get("Labeling Rule", "")))
        name = str(r.get("Condition", ""))
        selected_syms = set(st.session_state.user_data.get("selected_symptoms", []) or [])
        overlap_score, _ = _row_selected_overlap_score(selected_syms, r.get("Symptoms", ""))
        # Sort: stronger symptom fit, higher confidence, higher acuity, shorter rule, A→Z
        return (-overlap_score, -conf_rank, -acu, rule_len, name)
...
def evaluate_rule(rule_str: str, condition: dict, user_data: dict, *, phase: str = "selection") -> bool:
    ...
    def eval_atom(atom: str) -> bool:
        ...
        if m:
            return _selected_matches_atom(m.group(1).strip(), sel_syms)
...
def results_page():
    ...
    if recommendation:
        ...
        with col1:
            if st.button("🔄 Start Over", use_container_width=True):
                ...
                st.rerun()
        with col2:
            render_help_link("📅 Schedule a Consultation")
        return
    ...
    if condition.get("Referral"):
        render_help_link("📅 Schedule a Consultation or Get Help")
